...
function PayWithVNPay() {
    console.log("PayWithVNPay function called");
    let cusName = $("#customer_name").val();
    let cusAddress = $("#customer_address").val();
    let cusPhone = $("#customer_phone").val();
    let accountId = $("#accountId").val();

    console.log("Customer info:", { cusName, cusAddress, cusPhone, accountId });
    ...
    console.log("Elements found:", {
        quantityCount: elQuantity.length,
        productDetailIdCount: elProductDetailId.length,
        totalMoneyCount: elTotalMoney.length
    });
    ...
    if (confirm("Bạn có muốn thanh toán đơn hàng này qua VNPay không?")) {
        console.log("Sending AJAX request to /Order/CreateOrderWithVNPay");
        $.ajax({
            type: 'POST',
            url: "/Order/CreateOrderWithVNPay",
            contentType: 'application/json;charset=utf-8',
            data: JSON.stringify(obj),
            success: function (result) {
